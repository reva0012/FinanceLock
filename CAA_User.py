...
def open_caa_user():
	...
	def next_but():
		...
		if chk_cnt == 1 and t1:
			...
			acnt = db.child("People").child("Customer").get()
			for l in acnt:
				if l.val()['phone_no'] == e_pno.get():
					pid = l.key()
			storage.child("SalaryCert/" + pid + ".pdf").put(e_salc.get())	
	...
	click_emp = StringVar()
	click_emp.set("Yes")
	e_emp = OptionMenu(top_ca, click_emp, "Yes", "No")
	e_emp_win = my_canvas.create_window(200, 325, window = e_emp)
	select_but = Button(top_ca, text = "Select", command = sel, background = "white", highlightbackground = "black", highlightthickness = 2)
	...
	e_ocp = Entry(top_ca, width = 50, borderwidth = 2)
	e_ocp.insert(0, "Nil")
	e_ocp.config(state = "disabled")
