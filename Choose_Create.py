...
def chk_caa():
	...
	def choose():
		...
		if r_ch1.get() == 1 and r_ch2.get() == 0:
			...
			my_canvas.create_text(300, 300, text = "Choose 'Only One' Account Type", font = ("Helvetica", 15), fill = "steel blue")
	...
	next_button = Button (top_ch, text = "Next", command = choose)
	next_button_win = my_canvas.create_window(400, 250, window = next_button)

	goback_button = Button (top_ch, text = "Back", command = top_ch.destroy)
